...
int main() {
  ...
  if (network_init(&network, num_layers, layer_sizes) != 0) {
    fprintf(
        stderr,
        "An error has occurred while initializing the network. Aborting...\n");
    return EXIT_FAILURE;
  }

  printf("Network initialized with %d layers.\n", num_layers);

  Trainer *trainer = trainer_init(&network, LEARNING_RATE, EPOCHS, BATCH_SIZE,
                                  DATASET_SIZE, inputTrainData, inputLabelData);
